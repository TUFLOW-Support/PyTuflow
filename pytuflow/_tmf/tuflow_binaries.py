...
class TuflowBinaries:
    ...
    def _load_linux_packaged_tuflow(cls, cmd_search: list[str], cmd_query: list[str]) -> dict:
        ...
        try:
            output = subprocess.check_output(search, stderr=subprocess.PIPE, text=True, errors='ignore')  # nosec B603
            bin_names = [x.split('\t')[0] for x in output.splitlines()]
            for bin_name in bin_names:
                query = cmd_query + [bin_name]
                output = subprocess.check_output(query, stderr=subprocess.PIPE, text=True, errors='ignore')  # nosec B603
